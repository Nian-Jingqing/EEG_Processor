function [EEG, Warnings] = Proc_EpochEventSel(EEG, EventSelSettings, RejSettings, Warnings)
% -----
...
if EventSelSettings.UseEpochTime
    ...
    if EventSelSettings.SeparateFiles
        for i = 1:length(EventSelSettings.Labels)
            ...
            fprintf('>> BIDS: Removing %.3f s of rejected epochs in dataset %i of %i\n', sum(SelIdx & RejIdx)/EEG.srate, i, length(EventSelSettings.Labels))
            [PointOnset, PointDuration] = idx2bouts(SelIdx & ~RejIdx);
